...
//(if we save a pointer to the tail this operation would be O(1))
//space complexity: O(1)
...
Node* deleteFront(Node* head){
	...
	//because the linked list now is empty, on the other hand, we just update the
...
//time complexity: O(N) where N=size of the linked list
//(this could be O(1) if we save the tail)
//space complexity: O(1)
...
Node* deleteNode(Node* head, Node* loc){
	...
	if(head == loc){
		...
		//updating the next of the node before loc 
		(loc -> previous) -> next = loc -> next;
		//updating the previous of the node after loc
